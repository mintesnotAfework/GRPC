...
const (
	NumService_Sum_FullMethodName = "/number.NumService/Sum"
)
...
type NumServiceClient interface {
	Sum(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[NumRequest, NumResponse], error)
}
...
func (c *numServiceClient) Sum(ctx context.Context, opts ...grpc.CallOption) (grpc.ClientStreamingClient[NumRequest, NumResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &NumService_ServiceDesc.Streams[0], NumService_Sum_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type NumService_SumClient = grpc.ClientStreamingClient[NumRequest, NumResponse]
...
type NumServiceServer interface {
	Sum(grpc.ClientStreamingServer[NumRequest, NumResponse]) error
...
func (UnimplementedNumServiceServer) Sum(grpc.ClientStreamingServer[NumRequest, NumResponse]) error {
...
func _NumService_Sum_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(NumServiceServer).Sum(&grpc.GenericServerStream[NumRequest, NumResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type NumService_SumServer = grpc.ClientStreamingServer[NumRequest, NumResponse]
